...
def summary() -> None:
    """Print the values a correct rerun must reproduce, and the ones it may not.

    The point of separating them is that a reader can tell at a glance whether a rerun
    changed anything that matters. Guarantees, prices and ratios are properties of the
    linear programs and must not move; timings are properties of the machine and will.
    """
    ...
    print("    three-claimant vector       "
          + " ".join(f"{v:.4f}" for v in sorted_base) + "    expected 0.4195 0.4195 0.4477")
    ...
    print(f"    sigma_2 CTI-RLex / PROP-BR  {cache['cti_rlex']['sorted_rho'][1]:.4f} / "
          f"{cache['prop_br']['sorted_rho'][1]:.4f}        expected 0.1374 / 0.0564")
    print(f"    price of fairness           "
          f"{100 * (util['nominal_beneficial_delivery_af'] - rlex['nominal_beneficial_delivery_af']) / util['nominal_beneficial_delivery_af']:.2f}%"
          f"                  expected 2.57%")
    print(f"    value of recourse           "
          f"{100 * (rlex['minimum_guarantee'] - rigid['minimum_guarantee']) / rigid['minimum_guarantee']:.2f}%"
          f"                  expected 9.22%")
    print(f"    robustness cost             "
          f"{100 * (nominal['nominal_beneficial_delivery_af'] - rlex['nominal_beneficial_delivery_af']) / nominal['nominal_beneficial_delivery_af']:.1f}%"
          f"                  expected 31.5%")
    print(f"    factorial range             {min(factorial):.4f} to {max(factorial):.4f}     "
          f"expected 0.2243 to 0.7011")
